---
name: seo-static-site
description: Lightweight SEO audit and fix pass for STATIC HTML sites (hand-written HTML, GitHub Pages, plain Netlify/Vercel static deploys) with no build step. Runs a 9-point check over the local .html files and writes the fixes in place: title/meta/canonical, Open Graph, Twitter Cards, JSON-LD, robots.txt, sitemap.xml, image alt text, heading hierarchy. Use only when the site is static HTML on disk and the user wants edits applied directly; for hosted-URL analysis or framework sites use seo, seo-page or seo-audit instead.
---
