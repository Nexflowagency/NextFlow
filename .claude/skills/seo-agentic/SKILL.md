...
name: seo-agentic
description: >
  Script-backed, evidence-first SEO audit that runs bundled Python collectors
  (HTTP, HTML, schema, robots/sitemap, Core Web Vitals, hreflang) and returns
  prioritized findings with a confidence label and the raw evidence behind each
  one. Also audits GitHub repositories for discoverability. Use when the user
  wants reproducible, script-verified evidence rather than a model-only read, or
  says "agentic SEO", "evidence", "run the SEO scripts", or "repo SEO".
---
